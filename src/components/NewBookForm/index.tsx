...
export default function NewBookForm() {
    ...
    const onChangeFile = async (e: FormEvent<HTMLInputElement>) => {
        ...
        const responseBook = await fetch(`http://localhost:3333/upload/`, {
            method: 'POST',
            headers: {
    ...
    const onSubmit = async (e: FormEvent<HTMLInputElement>) => {
        ...
        const responseUser = await fetch(`http://localhost:3333/user-num-books/${userContext.user.id}`, {
            method: 'PATCH'
        });
        const parsedResUser = await responseUser.json();

        if(parsedResUser.num_books === 1) {
            const responseUser = await fetch(`http://localhost:3333/user-points/${userContext.user.id}`, {
                method: 'PATCH',
            });
